#!/usr/bin/env python3
"""
Run steps 6-12 of the Python pipeline using R's clone_nn_dis output.
"""
...
print(f"PCA: {pca.shape} | Cells: {n_cells} | Clones: {n_clones}")

# Load clone_nn_dis from R output
clone_nn_dis = pd.read_csv("tests/real_outputs_R/clone_nn_dis.csv")
print(f"clone_nn_dis: {len(clone_nn_dis)} rows, columns: {list(clone_nn_dis.columns)}")
...
print(f"Filtered cells: {n_kept}")

# Reconstruct sparse matrix (triplets are 1-indexed)
...
print(f"cell_clone_prob_sparse: {cell_clone_prob_sparse.shape}, nnz={cell_clone_prob_sparse.nnz}")

# === Convert clone_nn_dis to square matrix ===
print("\n=== Converting clone_nn_dis to square matrix ===")
clone_dis_sq = ct.long2square(
    ...
    symmetric=True,
    na_fill=0.0,
    nodes=list(range(1, n_clones + 1))  # R uses 1-indexed clone IDs
)
np.fill_diagonal(clone_dis_sq, 0)
print(f"clone_dis_sq: {clone_dis_sq.shape}")
...
print("\n=== Step 6: Clone MDS (30 dims) ===")
np.random.seed(42)
# Use metric MDS first, then refine with non-metric.
# sklearn's non-metric SMACOF degenerates at k>=5 with random init;
# R's MASS::isoMDS uses Kruskal's algorithm which doesn't have this issue.
...
print(f"MDS embedding: {clone_embedding_df.shape}")
